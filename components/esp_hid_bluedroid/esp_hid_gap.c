 ...
 * BLE GAP
 * */
extern void do_when_ble_hid_connect(void);
...
#if CONFIG_HID_DEV_AUTHENTICATION_METHOD
    esp_ble_io_cap_t iocap = CONFIG_HID_DEV_AUTHENTICATION_METHOD;
#else
...
#if CONFIG_HID_AUTHENTICATION_KEY
    uint32_t passkey = CONFIG_HID_AUTHENTICATION_KEY;
#else
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    fields.appearance = appearance; // ESP_HID_APPEARANCE_GENERIC; // DDDGR_test
    ...
    ble_hs_cfg.sm_io_cap = BLE_SM_IO_CAP_NO_IO; // df = BLE_SM_IO_CAP_DISP_ONLY;
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP)
        {
            ...
            pkey.passkey = 888888;// 123456; // This is the passkey to be entered on peer // DDDGR_test
