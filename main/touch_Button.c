...
void touch_do_initial_scanning(const char *_TAG, touch_sensor_handle_t sens_handle, touch_channel_handle_t chan_handle)
{
    ...
    for (int i = 0; i < TOUCH_CHAN_INIT_SCAN_TIMES; i++)
        ESP_ERROR_CHECK_WITHOUT_ABORT(touch_sensor_trigger_oneshot_scanning(sens_handle, 2000));
    ...
    {
        uint32_t new_active_thresh = (uint32_t)(benchmark[j] * s_thresh2bm_ratio);
        ESP_LOGI(_TAG, " Touch sample [%d]: %"PRIu32", %"PRIu32"\t", j, benchmark[j], new_active_thresh);

        new_chan_cfg.active_thresh[j] = new_active_thresh;
    }

    /* Update the channel configuration */
    ESP_ERROR_CHECK_WITHOUT_ABORT(touch_sensor_reconfig_channel(chan_handle, &new_chan_cfg));
}
...
esp_err_t init_TouchBuitin(const char *_TAG, int touch_channel_id, EventGroupHandle_t xEventGroup)
{
    ...
    ESP_RETURN_ON_ERROR(touch_sensor_new_controller(&sens_cfg, &sens_handle), _TAG, "Failed to init touch controller.");

    touch_channel_config_t chan_cfg = TOUCH_CHAN_CFG_DEFAULT();
    ESP_RETURN_ON_ERROR(touch_sensor_new_channel(sens_handle, touch_channel_id, &chan_cfg, &chan_handle), _TAG, "Failed to init touch channel handle.");

    touch_sensor_filter_config_t filter_cfg = TOUCH_SENSOR_DEFAULT_FILTER_CONFIG();
    ESP_RETURN_ON_ERROR(touch_sensor_config_filter(sens_handle, &filter_cfg), _TAG, "Failed to config filter.");
    ...
    // ESP_RETURN_ON_ERROR(touch_sensor_register_callbacks(sens_handle, &callbacks, NULL), _TAG, "Failed to register callbacks.");
    ESP_RETURN_ON_ERROR(touch_sensor_register_callbacks(sens_handle, &callbacks, (void *)xEventGroup), _TAG, "Failed to register callbacks.");
    ESP_RETURN_ON_ERROR(touch_sensor_enable(sens_handle), _TAG, "Failed to enable touch.");
    ESP_RETURN_ON_ERROR(touch_sensor_start_continuous_scanning(sens_handle), _TAG, "Failed to start scanning.");
...
{
    ESP_RETURN_ON_ERROR(touch_sensor_stop_continuous_scanning(sens_handle), _TAG, "Failed to stop scanning.");
    ESP_RETURN_ON_ERROR(touch_sensor_disable(sens_handle), _TAG, "Failed to disable touch.");
    ...
    ESP_RETURN_ON_ERROR(touch_sensor_config_sleep_wakeup(sens_handle, &slp_cfg), _TAG, "Failed to config sleep wakeup.");
...
esp_err_t init_TouchExternal(const char *_TAG, gpio_num_t pin_touch_sensor)
{
    ...
    gpio_config_t io_config = {
        .pin_bit_mask = BIT(pin_touch_sensor),
        .mode = GPIO_MODE_INPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE
    };
    ESP_RETURN_ON_ERROR(gpio_config(&io_config), _TAG, "Failed to setup touch input pin.");
