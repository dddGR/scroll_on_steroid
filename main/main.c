...
#include "freertos/task.h"      // IWYU pragma: export
#include "sdkconfig.h"
...
void task_TouchScan(void* pvParameter) {
    ...
    for ( ;; ) {
        currbits = (gpio_get_level(CONFIG_PIN_TOUCH_SENSOR)) ? BUTTON_PRESSED
                                                             : BUTTON_RELEASED;
...
static void task_Button(void* pvParameter) {
    ...
    for ( ;; ) {
        xEventGroupWaitBits(g_xEventGroup,
                            DEVICE_IDLE | BLE_CONNECTED,
                            pdFALSE,
                            pdTRUE,
                            portMAX_DELAY);
        ...
        switch ( getButtonPressState(g_xEventGroup, hdl_scroll_task) ) {
            case B_PRESS_2: {
                g_scroll_dir = (g_scroll_dir == SCROLL_HORIZONTAL)
                    ? SCROLL_VERTICAL
                    : SCROLL_HORIZONTAL;
                ESP_LOGI(TAG, "[double pressed] - Change scroll direction.");
            } break;
            case B_PRESS_3: {
                g_scroll_div = (g_scroll_div == SPEED_HIGH) ? SPEED_LOW
                                                            : SPEED_HIGH;
                ESP_LOGI(TAG,
                         "[triple pressed] - Change scroll speed to %s",
                         (g_scroll_div == 1) ? "HIGH" : "LOW");
            } break;
            case B_PRESS_NONE: {
                xEventGroupWaitBits(g_xEventGroup,
                                    DEVICE_ACTIVE,
...
static esp_err_t init_Hardware(const char* TAG,
    ...
    gpio_config_t pin_config = {.pin_bit_mask = BIT(CONFIG_PIN_SENSOR_POWER),
                                .mode         = GPIO_MODE_OUTPUT,
                                .pull_up_en   = GPIO_PULLUP_DISABLE,
                                .pull_down_en = GPIO_PULLDOWN_DISABLE,
                                .intr_type    = GPIO_INTR_DISABLE};
    ESP_RETURN_ON_ERROR(gpio_config(&pin_config),
                        TAG,
                        "failed to configure power pin..");
    ESP_RETURN_ON_ERROR(gpio_set_level(CONFIG_PIN_SENSOR_POWER, 1),
                        TAG,
     ...
     */
#if CONFIG_USE_TOUCH_BUILTIN
    ESP_LOGI(TAG, "Initializing Internal TouchSensor...");
    ESP_ERROR_CHECK(init_TouchBuitin(TAG, CONFIG_TOUCH_PAD_ID, g_xEventGroup));
#else
    xTaskCreatePinnedToCore(&task_TouchScan,
                            "task_TouchScan",
                            2560,
                            (void*)g_xEventGroup,
                            6,
                            &hdl_touch_task,
                            (BaseType_t)APP_CORE_ID);
...
static void task_MouseScroll(void* pvParameter) {
    ...
    xEventGroupSetBits(g_xEventGroup,
                       DEVICE_IDLE
...
static void esp_pre_sleep_config(const char* TAG) {
    ...
    const gpio_config_t config = {.pin_bit_mask = BIT(CONFIG_PIN_WAKEUP),
...
void app_main(void) {
    ...
    xTaskCreatePinnedToCore(&task_MouseScroll,
                            "task_MouseScroll",
                            3584,
                            NULL,
                            5,
    ...
    /* everything will be config in `task_MouseScroll`,
     * main task now will wait 5 seconds (magic number),
     * and then continue to handle sleeping task. */
    ...
    for ( ;; ) {
        ...
        if ( bits & BLE_DISCONNECTED ) {
            ESP_LOGW(MTAG,
                     "BLE not connected, wait 30 seconds until go to sleep.");
            bits = xEventGroupWaitBits(g_xEventGroup,
        ...
        ESP_LOGW("[SCROLL]",
                 "high watermark %d",
                 uxTaskGetStackHighWaterMark(hdl_scroll_task));
#if CONFIG_USE_TOUCH_BUTTON
    #if !CONFIG_USE_TOUCH_BUILTIN
        ESP_LOGW("[TOUCH]",
                 "high watermark %d",
                 uxTaskGetStackHighWaterMark(hdl_touch_task));
    #endif
        ESP_LOGW("[BUTTON]",
                 "high watermark %d",
                 uxTaskGetStackHighWaterMark(hdl_button_task));
#endif
